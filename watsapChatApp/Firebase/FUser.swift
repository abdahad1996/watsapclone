...
class FUser {
    ...
    //initially create user with parameters not filled and save it locally and on firestore
    ...
    class func logOutCurrentUser(completion: @escaping (_ success: Bool) -> Void) {
        ...
        removeOneSignalId()
        // we locally remove user from device
...
func saveUserLocally(fUser: FUser) {
    ...
    // to save the changes in dictionary immediately  before automatic save
...
//updates user in firestore and locally using userdefaults by first creating copy of data from userdefault , updating the copy and then storing it in userdefaults
func updateCurrentUserInFirestoreandinUserdefault(withValues : [String : Any], completion: @escaping (_ error: Error?) -> Void) {
    
    if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
        print("userdefault old dic")
        print(dictionary)
        ...
        // creates a copy of locally saved dictionary from userdefault

        let userObject = (dictionary as! NSDictionary).mutableCopy() as! NSMutableDictionary
        
        //the local dict copy from userdefault checks for changes using keys from tempdic and updates them so we have updated copy now. it is still not saved in user default
        userObject.setValuesForKeys(tempWithValues)
        print("userdefault updated dic")
        print(userObject)
        
        
        //update data takes temp dict and updates the database. if no database exist den we get error
        reference(.User).document(currentUserId).updateData(withValues) { (error) in
            ...
            //the updated copy is now set in userdefault with key kcurrentuser
...
func updateCurrentUserOneSignalId(newId: String) {
    ...
    updateCurrentUserInFirestoreandinUserdefault(withValues: [kPUSHID : newId]) { (error) in
        if error != nil {
