...
class FinishRegistrationViewController: UIViewController, ImagePickerDelegate {
    ...
    // we get this from welcomeViewController
    var email: String!
    var password: String!
    ...
    // cancel and go back to welcome view
...
//            self.registerUser()
    ...
    func registerUser() {
        ...
        // our user didnt choose avatar
        if avatarImange == nil {
            //creates image from initials
            imageFromInitials(firstName: nameTextField.text!, lastName: surnameTextField.text!) { (avatarInitials) in
                //convert image to data
                let avatarIMG = avatarInitials.jpegData(compressionQuality: 0.7)
                //convert data to string
                let avatar = avatarIMG!.base64EncodedString(options: NSData.Base64EncodingOptions(rawValue: 0))
                //set kavatar key to avatar in temp dict
            ...
            //our user choose avatar so we have avatar image
        } else {
    ...
    //updated dict is stored locally and in firestore
    func finishRegistration(withValues: [String : Any]) {
        
        updateCurrentUserInFirestoreandinUserdefault(withValues: withValues) { (error) in
